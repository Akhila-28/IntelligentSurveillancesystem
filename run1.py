...
from imutils.video import VideoStream, FPS # type: ignore
from scipy.spatial import distance as dist # type: ignore
import numpy as np # type: ignore
import argparse, imutils, cv2, os, time, schedule # type: ignore
...
# get the unconnected out layers and check the type
unconnected_layers = net.getUnconnectedOutLayers()

# check if the first element is iterable or not
if isinstance(unconnected_layers[0], np.ndarray):
    # handle case where it is a list of lists (2D array)
    ln = [ln[i[0] - 1] for i in unconnected_layers]
else:
    # handle case where it is a list (1D array)
